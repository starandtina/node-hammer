...
function * runJob(next) {
  ...
  for (var i = 0, l = databases.length; i < l; i++) {
    ...
    r[db].tpm = ~~ (e.tpm - s.tpm);
    r[db].tpmC = ~~ (e.tpmC - s.tpmC);
  }
...
function * compute(dbConfig, databases) {
  ...
  for (var i = 0, l = databases.length; i < l; i++) {
    dbConfig.database = databases[i];
    var query = yield util.createQuery(dbConfig);
    var tpm = yield getTPM(query);
    var tpmC = yield getTPMC(query);
...
function * respond(next) {
  ...
  var basename = path.basename(this.config.jobSpec);
  yield writeFile(path.resolve(outputDir, basename + '_result.yml'), yaml.safeDump(this.results));
}

function * getTPM(query) {
  return~~ (yield query('select sum(execution_count) from sys.dm_exec_query_stats'))[0][''];
}

function * getTPMC(query) {
  return~~ (yield query('select sum(d_next_o_id) from district'))[0][''];
}