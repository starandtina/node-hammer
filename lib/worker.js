...
process.on('message', function (m) {
    ...
    var config = m.config;
    co(function * () {
        var connection = yield util.createConnection(config);
        var query = yield util.createQuery(connection);
        var max_w_id = yield getMaxWarehouseId(query);
        var max_d_id = yield getMaxDistrictId(query);
...
function * getMaxWarehouseId(query) {
    return~~ (yield query('select max(w_id) from warehouse'))[0][''];
}

function * getMaxDistrictId(query) {
    return~~ (yield query('select max(d_id) from district'))[0][''];
}
...
function * executeNEWORD(request, w_id, max_w_id) {
    ...
    var execute = thunk(request.execute, request);
